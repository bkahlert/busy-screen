...
+main() {
    ...
    if command -v pihero &>/dev/null; then
        local result
        CHECKS_OUTPUT_FORMAT=none COLUMNS=$columns run_command_line pihero diag
        result=$?
        tput civis || true
        [ "$result" -le "$exit_code" ] || exit_code=$result
        summary+=("$(summary_component "$result" pihero)")
    fi

    case ${CHECKS_OUTPUT_FORMAT:-ansi} in
    none) : ;;
    markdown) gum format --type template '' " $(wizard_template --mood="$exit_code" || true) ${summary[*]}" '' '' ;;
    *) gum format --type template '' " $(wizard_template --mood="$exit_code" || true) ${summary[*]}" '' '' ;;
    esac
